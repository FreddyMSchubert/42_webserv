...
void Response::handle_file_req(t_server_config &config, Request &req)
{
	...
	try
	{
		std::string file = getFileAsString(Path::combinePaths(config.default_location.root, path));
...
void Response::handle_dir_req(t_server_config &config, Request &req)
{
	...
	std::vector<std::filesystem::directory_entry> entries = getDirectoryEntries(Path::combinePaths(config.default_location.root, req.getPath()));
