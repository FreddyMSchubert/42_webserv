...
Path::Path(std::string path, Type type, t_server_config &config) : _config(&config)
{
	...
	else
	{
		...
		if (path.find(_config->default_location.root.asFilePath()) != 0)
	...
	if (!std::filesystem::exists(Path::combinePaths(_config->default_location.root.asFilePath(), _path)))
...
std::string Path::asFilePath() const
{
	if (!_config)
		throw std::runtime_error("Path: path: config is nullptr");
	return combinePaths(_config->default_location.root.asUrl(), _path);
}
...
void Path::goDownIntoDir(const std::string& dir)
{
	...
	std::string new_path_filesystem = Path::combinePaths(_config->default_location.root.asFilePath(), new_path);
