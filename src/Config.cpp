...
{
	std::regex pattern(R"(^[ \t]*error_page[ \t]+([1-5][0-9]{2})[ \t]+(\S+);$)");
	(void)iter;
	...
	for (size_t i = 0; i <= tmp_index.size() - 1; i++)
	{
		...
		tmp_index[tmp_index.size() - 1].erase(
			std::remove(tmp_index[tmp_index.size() - 1].begin(), tmp_index[tmp_index.size() - 1].end(), ';'),
			tmp_index[tmp_index.size() - 1].end()
		);
		server.error_pages.emplace(
		atoi_value, 
		FilePath((tmp_index[tmp_index.size() - 1]), Path::Type::URL, &server)
		);
	}
}
...
void	init_index_file(std::string str, t_server_config & server, int & iter)
{
	...
	for (size_t i = 0; i < tmp_index.size(); i++)
	{
		...
		{
			std::cout << "Exception for Config: " << ex.what() << std::endl;
			if (i == tmp_index.size() - 1)
				throw std::runtime_error("None of the index files are valid: \"" + str + "\"");
			continue;
		}
	}
	if (server.index_file.isEmpty())
		throw std::runtime_error("Write a better config you dick sucking cok.");
}
...
void	init_redirection(t_server_config & server, t_location & location, std::string str)
{
	...
	location.redirections.emplace(
    std::atoi(str_for_atoi.c_str()), 
	FilePath(post_processed_redirection[post_processed_redirection.size() - 1], Path::Type::URL, &server)
	);
}

void	init_root(t_server_config & server, t_location & location, std::string str)
{
...
void	init_upload_dir(t_server_config & server, t_location & location, std::string str)
{
...
void	read_location(t_server_config & server, std::vector<std::string> & current_server, size_t start_of_location, size_t end_of_location, std::string location_str)
{
	...
	while (start_of_location < end_of_location && start_of_location < current_server.size())
	{
		if (std::regex_match(current_server[start_of_location], allowed_methods))
			...
			init_redirection(server, location, current_server[start_of_location]);
		else if (std::regex_match(current_server[start_of_location], root))
			init_root(server, location, current_server[start_of_location]);
			...
			init_upload_dir(server, location, current_server[start_of_location]);
		else
